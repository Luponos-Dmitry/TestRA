...
Функция aboutme(Запрос) Экспорт
	...
	ОтветСодержимое.Вставить("authuser", XMLСтрока(ПользовательСтруктура.Ссылка) + "( имя: " + ПользовательСтруктура.Наименование + ")");
...
Функция PipeMessFile(ВходящийЗапрос) Экспорт
	...
	Пока Результат.Следующий() Цикл  
		ОтветСодержимое.Добавить(Новый Структура("user,guid,message",Результат.Пользователь,Результат.GuidСообщения,Результат.ТекстСообщения));	
	КонецЦикла;
	...
	ОтветСодержимое.Добавить(Новый Структура("authuser,externalUser,name",GUIDПользователя,ЭтоВнешнийПользователь,ПользовательСтруктура.Наименование));
	// --- Лупонос Д.В. 2024-08-27 Добавлены идентификационные данные пользователя системы для отладки 
	ОтветJSON = Новый ЗаписьJSON; 
	ОтветJSON.УстановитьСтроку(Новый ПараметрыЗаписиJSON(,Символы.Таб));
	ЗаписатьJSON(ОтветJSON,ОтветСодержимое); 
