...
Процедура УдалитьСессиюКонтрагента(Контрагент)
	...
	ЗаписьСессии.Контрагент = Контрагент;
...
Процедура ПрочитатьИОбработатьСообщение(Сообщение)
	...
	Если Сообщение.ТипСообщения = Перечисления.ТипыСообщения.callbackquery Тогда	// Это пришёл запрос доступа
		...
		Пока Выборка.Следующий() Цикл
			...
			ОтправитьСообщение(Выборка.Контрагент.ТелеграмID, СтрШаблон("Поступил запрос на предоставление доступа от [%1](http://t.me/%2).", Сообщение.Контрагент.ТелеграмИмя, Сообщение.Контрагент.ТелеграмНик), , ПараметрыЗапроса);
		КонецЦикла;
